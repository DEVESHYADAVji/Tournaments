...
const Bracket: React.FC = () => {
  ...
  const load = React.useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [tournamentData, response] = await Promise.all([
        getTournamentById(id),
        httpClient.get(`/tournaments/${Number(id)}/bracket`),
      ]);
      setTournament(tournamentData);
      setSlots(response.data as BracketSlot[]);
    } catch (error) {
      setMessage((error as AxiosError<{ detail?: string }>)?.response?.data?.detail || 'Unable to load the generated bracket.');
    } finally {
      setLoading(false);
    }
  }, [id]);
  ...
  if (loading) return <div className="loading-view"><div className="spinner" aria-hidden="true" /><p>Loading bracket...</p></div>;
  ...
  return <div className="section-stack page-enter">
    <section className="hero-surface"><div className="hero-inner"><p className="section-label">Bracket</p><h1 className="page-title">{tournament.name}</h1><p>{tournament.format} · Generated from checked-in participants.</p><div className="inline-actions"><Link to={`/tournaments/${tournament.id}`} className="btn btn-secondary">Back to tournament</Link><span className="small-pill">Participant view</span></div>{message ? <p className="message-text" role="alert">{message}</p> : null}</div></section>

...
interface Order { id: number; tournament_id: number; order_id: string; amount: number; currency: string; status: string; }
  ...
  const pay = async () => {
    ...
    try {
      ...
      const checkout = new window.Razorpay({
        key: config.key_id, amount: order.amount * 100, currency: order.currency, name: 'Tournaments', description: `Tournament entry #${tournamentId}`, order_id: order.order_id,
        handler: async (result: { razorpay_order_id: string; razorpay_payment_id: string; razorpay_signature: string }) => {
          try {
            ...
            setPayments((await httpClient.get('/payments/me')).data as Order[]);
          } catch (error) {
